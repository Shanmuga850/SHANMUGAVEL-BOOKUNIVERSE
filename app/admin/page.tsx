...
async function uploadDirect(file: File, bucket: 'ebooks' | 'covers'){
  ...
  async function handlePublishAudiobook(){
    ...
    try{
      ...
      const openingRes = audioOpening? await upload(audioOpening!, 'audio-opening') : null
      const endingRes = audioEnding? await upload(audioEnding!, 'audio-ending') : null

      const chapterRes = []
      for(let i=0;i<15;i++){
        if(audioChapters[i]){
          const cr = await upload(audioChapters[i]!, `audio-chapter-${i+1}`);
      ...
      <div className="p-6 max-w- mx-auto">
        ...
        {tab==='audiobooks' && (
          <div className="rounded- bg-black border border-[#D4AF37]/20 p-6">
            ...
            <div className="mt-8 flex gap-3"><button onClick={handlePublishAudiobook} disabled={audioUploading} className="h-11 px-8 rounded-full bg-[#D4AF37] text-black font-bold text- uppercase flex items-center gap-2"><CoinLogo size={20}/>{audioUploading?'Uploading...':'Publish Audiobook • GOLD BOX'}</button></div>
          </div>
        )}
