...
// === SPECIFIC PLACE 1: uploadDirect — FIXED to support audiobooks ===
...
// === SPECIFIC PLACE 2: handlePublishAudiobook — DELETED fetch('/api/upload') — NOW DIRECT ONLY ===
  async function handlePublishAudiobook(){
    if(!audioTitle||!audioCover) return alert('Title + Cover Mandatory only!')
    ...
    try{
      ...
      for(let i=0;i<15;i++){
        if(audioChapters[i]){
          ...
          chapterRes.push({ no: i+1, url: cr.url, path: cr.path })
        }
      }
      ...
      if(error) throw error
      alert(`Audiobook Published! 🎧 ${chapterRes.length} Audios Live!`)
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="p-6 max-w- mx-auto">
        ...
        {tab==='audiobooks' && (
          <div className="rounded- bg-black border border-[#D4AF37]/20 p-6">
            <h2 className="font-serif-lux text- font-bold text-[#D4AF37]">Create Audiobook — GOLD BIG BOX — DIRECT NO API</h2>
            <div className="grid md:grid-cols-2 gap-6 mt-6">
              <div><label className="text- uppercase tracking-widest text-[#D4AF37]">Title*</label><input value={audioTitle} onChange={e=>setAudioTitle(e.target.value)} placeholder="Gun Story Volume 1" className="mt-1 w-full h-11 px-4 rounded- bg-black border border-white/10 text-"/></div>
