    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] grid place-items-center p-6">
        <div className="w-full max-w- rounded- bg-black border border-[#D4AF37]/20 p-8 text-center">
          ...
          {step===2 && (<div className="mt-8 text-left"><label className="text- uppercase tracking-widest text-[#D4AF37]">Step 2: Email / Password</label><input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="Email" className="mt-3 w-full h-11 px-4 rounded-full bg-black border border-white/10 text-" /><input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} placeholder="Password" className="mt-3 w-full h-11 px-4 rounded-full bg-black border border-white/10 text-" /><button onClick={()=>{if(email.trim()==='shanmugavelvetri@gmail.com'&&pwd==='VelShanmugam@850') setStep(3); else alert('Invalid')}} className="mt-4 w-full h-11 rounded-full bg-[#D4AF37] text-black font-bold text- uppercase">Next</button></div>)}
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="p-6 max-w- mx-auto">
        {tab==='mybooks' && (
          <div>
            <div className="flex items-center gap-3 mb-6"><CoinLogo size={48}/><div><h1 className="font-serif-lux text- font-bold">My Books • {myEbooks.length}</h1><p className="text- uppercase tracking-widest text-[#D4AF37]/60">Total Ebooks: {myEbooks.length}</p></div></div>
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="rounded- bg-black border border-[#D4AF37]/20 p-8 text-center text- text-white/30">Audiobooks next — P4</div>
            </div>
          </div>
        )}
        {tab==='ebooks' && (
